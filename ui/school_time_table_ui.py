...
class ShoolTimeTableUI:
    ...
    def __init__(self, window: QtWidgets.QMainWindow):
        window.setGeometry(0, 0, 800, 1000)
    ...
    def initailize(self, members, iniMemberScedual):
        ...
        for i, classes in enumerate(iniMemberScedual):
            for classTime in classes[1]:
                self.Labels[classTime[len(classTime) :: -1]].setText(
                    classes[0]
                )  # reverse
                self.Labels[classTime[len(classTime) :: -1]].setStyleSheet(
                    self.showupstyle_Labels
                    + "background:"
                    + self.pick_background_color[i]
                    + ";"
                )
