...
class CleaningTimeTableUI:
    ...
    def __init__(
        ...
        self.member_work = [
            [
                member + "倒垃圾",
                member + "掃+拖",
                self.pickOneColorForaMember[self.members.index(member)],
            ]
    ...
    def setupUI(self, month):
        ...
        if (
            date(2023, month, 1).isoweekday() == 6
            or date(2023, month, 1).isoweekday() == 7
        ):
            self.first_monday = -1
            for i in range(7):
            ...
            for i in range(1, 6):
                for j in range(5):
                    label_name = str(i) + "," + str(j)
                    today = date(2023, month, 1) + timedelta(
                        days=(i - 1) * 7 + j + first_monday - 1
                    )
                    if today.strftime("%m") == nextMonth:  # 換月了
                        ...
                        self.Labeldays[label_name].setText(
                            (
                                date(2023, month, 1)
                                + timedelta(days=(i - 1) * 7 + j + first_monday - 1)
                            ).strftime("%d")
                        )
            ...
            for i in range(1, 6):
                for j in range(5):
                    ...
                    try:
                        ...
                        pass
                    if date(2023, today_month, today_day) < date(
                        2023, self.semesterStartD[0], self.semesterStartD[1]
                    ) or date(2023, today_month, today_day) > date(
                        2023, self.semesterEndD[0], self.semesterEndD[1]
                    ):
                        break
                    if j == 2:
                        self.Label2s[label_name].setStyleSheet(
                            """
                        background:"""
                            + self.member_work[count_member][2]
                            + """;
                        """
                        )
                        self.Label2s[label_name].setText(
                            self.member_work[count_member][1]
                        )
                        if count_member == 7:
                            count_member = 0
                        else:
                    ...
                    if two_days_flag == 1 or two_days_flag == 2:
                        ...
                        self.Label1s[label_name].setText(
                            self.member_work[count_member][0]
                        )
                        two_days_flag += 1
                        if count_member == 7:
                            count_member = 0
            ...
            for i in range(1, 6):
                for j in range(5):
                    ...
                    if i == 1 and j < dayOftheWeek - 1:
                        continue
                    today = date(2023, month, 1) + timedelta(
                        days=(i - 1) * 7 + (j + 1) - (dayOftheWeek - 1) - 1
                    )
                    if today.strftime("%m") == nextMonth:  # 換月了
                        self.Labeldays[label_name].setText(today.strftime("%m/%d"))
                        self.Labeldays[label_name].setStyleSheet(self.style_Labelday)
            ...
            for i in range(1, 6):
                for j in range(5):
                    ...
                    try:
                        ...
                        pass
                    if (
                        date(2023, today_month, today_day)
                        < date(2023, self.semesterStartD[0], self.semesterStartD[1])
                    ) or (
                        date(2023, today_month, today_day)
                        > date(2023, self.semesterEndD[0], self.semesterEndD[1])
                    ):
                        break
                    label_name = str(i) + "," + str(j)
                    if i == 1 and j < dayOftheWeek - 1:
